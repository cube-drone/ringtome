...
just ui-check                 # rebuild the JS/CSS bundles + eslint (esbuild can't tell a
                              # typo from a browser global; no-undef is the gate that can -
                              # js/eslint.config.js) + the UI's pure-module tests: no
                              # browser, no node, ~2s

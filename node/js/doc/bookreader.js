...
const Tree = ({ section, root, book, page, depth }) => html`<ul class=${depth === 0 ? 'book-reader-list book-reader-list-top' : 'book-reader-list'}>
    ${section.pages.map(
        (p) => html`<li class=${p.post === page ? 'book-reader-page book-reader-page-current' : 'book-reader-page'} key=${p.post}>
            <a href=${`/id/${root}/post/${book}/${p.post}`}>${p.title || t('doc.bookreader.untitled-page', 'untitled page')}</a>
        </li>`
    )}
    ${section.sections.map(
        (s, i) => html`<li class="book-reader-section" key=${`${depth}-${i}`}>
            ...
            <${Tree} section=${s} root=${root} book=${book} page=${page} depth=${depth + 1} />
        </li>`
    )}
</ul>`;
...
export const BookReader = ({ root, book, page: asked, title }) => {
    ...
    return html`<section class="book-reader">
        <aside class="book-reader-tree">
            ...
            <${Tree} section=${{ pages: payload.pages, sections: payload.sections }} root=${root} book=${book} page=${page} depth=${0} />
        </aside>
        <div class="book-reader-pane">
            <nav class="book-reader-nav">
            ...
            ${page &&
            html`<article class="book-reader-article">
                ...
                <p class="book-reader-permalink"><a href=${`/id/${root}/post/${page}`}>${t('doc.bookreader.this-page-on-its-own', 'this page on its own')}</a></p>
            </article>`}
        </div>
    </section>`;
};

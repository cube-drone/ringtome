...
fn user_db_opens_are_deliberate() {
    let expected: BTreeMap<&str, usize> = BTreeMap::from([
        ...
        ("identity.rs", 4),
        ("idface.rs", 14), // 13: the whole-shelf read behind a search or the facets (2026-09-07); 14: the shares count for the kind row (2026-09-08) - once per request each          // + stored_tree_leaves: ONE mirror per revalidation;

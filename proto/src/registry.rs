...
pub struct ChatMessage {
    ...
    pub body: Vec<u8>,
    pub sealed: bool,
    /// The media twins the body embeds (CHAT.md, ruling 11): the message's `refs`, as a
    /// header's - a claim about the body, derived at say time from what the bake minted,
    /// capped as a header's is (`DocHeaderPlain::MAX_REFS`), and absent on the wire when
    /// empty, so a message without media encodes as it always has.
    pub refs: Vec<[u8; 16]>,
    /// The personas the body's user cards name (CHAT.md, slice 6): derived at say time,
    /// never the speaker themself, capped, absent when empty. In the clear on a sealed
    /// room's chain - which travels only under the room's door - so the notice a mention
    /// rings can be checked by its recipient without the room's key.
    pub mentions: Vec<[u8; 32]>,
    /// The message this one answers with an emoji (CHAT.md, slice 9): the target's entry
    /// hash. A reaction is an ordinary message on the reactor's own room chain whose body
    /// is the emoji's shortcode; the floor stacks it under the target rather than showing
    /// it as a line. Absent for a line of talk.
    pub reacts_to: Option<[u8; 32]>,
    /// An earlier entry of this speaker's that this one takes back (CHAT.md, slices 8 and
    /// 9): a reaction withdrawn, later a line deleted. The chain never forgets; the memo
    /// stops counting. Absent for anything said rather than unsaid.
    pub retracts: Option<[u8; 32]>,
}
...
impl ChatMessage {
    ...
    fn well_formed(&self) -> Result<(), ProtoError> {
        ...
        if self.refs.len() > DocHeaderPlain::MAX_REFS {
    ...
    pub fn encode(&self) -> Result<Vec<u8>, ProtoError> {
        self.well_formed()?;
        let mut w = Writer::new();
        w.map(3 + u64::from(!self.refs.is_empty()) + u64::from(!self.mentions.is_empty()) + u64::from(self.reacts_to.is_some()) + u64::from(self.retracts.is_some()));
        w.uint(0);
        ...
        if !self.mentions.is_empty() {
            w.uint(4);
            w.array(self.mentions.len() as u64);
            for m in &self.mentions {
                w.bytes(m);
            }
        }
        if let Some(target) = &self.reacts_to {
            w.uint(5);
            w.bytes(target);
        }
        if let Some(earlier) = &self.retracts {
            w.uint(6);
            w.bytes(earlier);
        }
        Ok(w.into_bytes())
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, ProtoError> {
        ...
        while let Some(k) = map.next_key()? {
            match k {
                ...
                4 => {
                    let n = map.array()?;
                    if n as usize > Self::MAX_MENTIONS {
                        return Err(ProtoError::BadEntry("a chat message names too many people"));
                    }
                    for _ in 0..n {
                        mentions.push(map.bytes_fixed::<32>()?);
                    }
                }
...
mod tests {
    ...
    fn a_chat_message_round_trips_and_refuses_silence() {
        let m = ChatMessage { room_author: [7u8; 32], body: b"hello the room".to_vec(), sealed: false, refs: Vec::new(), mentions: Vec::new(), reacts_to: None, retracts: None };
        assert_eq!(ChatMessage::decode(&m.encode().unwrap()).unwrap(), m);
        let sealed = ChatMessage { room_author: [7u8; 32], body: vec![0xaa; 40], sealed: true, refs: Vec::new(), mentions: Vec::new(), reacts_to: None, retracts: None };
        assert_eq!(ChatMessage::decode(&sealed.encode().unwrap()).unwrap(), sealed);
        let silent = ChatMessage { room_author: [7u8; 32], body: Vec::new(), sealed: false, refs: Vec::new(), mentions: Vec::new(), reacts_to: None, retracts: None };
        assert!(silent.encode().is_err());
        let long = ChatMessage { room_author: [7u8; 32], body: vec![b'x'; ChatMessage::MAX_BODY_BYTES + 65], sealed: false, refs: Vec::new(), mentions: Vec::new(), reacts_to: None, retracts: None };
        assert!(long.encode().is_err());
    }

    /// CHAT.md ruling 11: a message's refs round-trip, no refs is wire-absence (byte-equal to
    /// a pre-refs encoding, so every message said before media agrees with itself), and the
    /// cap refuses at both doors.
    #[test]
    fn a_chat_message_carries_its_media_refs() {
        let with = ChatMessage { room_author: [7u8; 32], body: b"look".to_vec(), sealed: false, refs: vec![[1u8; 16], [2u8; 16]], mentions: vec![[9u8; 32]], reacts_to: Some([4u8; 32]), retracts: Some([5u8; 32]) };
        assert_eq!(ChatMessage::decode(&with.encode().unwrap()).unwrap(), with);
        let without = ChatMessage { room_author: [7u8; 32], body: b"look".to_vec(), sealed: false, refs: Vec::new(), mentions: Vec::new(), reacts_to: None, retracts: None };
        ...
        let over = ChatMessage { room_author: [7u8; 32], body: b"look".to_vec(), sealed: false, refs: vec![[1u8; 16]; DocHeaderPlain::MAX_REFS + 1], mentions: Vec::new(), reacts_to: None, retracts: None };
        ...
        forged.uint(0);
        forged.uint(3);
        forged.array((DocHeaderPlain::MAX_REFS + 1) as u64);
        for _ in 0..=DocHeaderPlain::MAX_REFS {
            forged.bytes(&[1u8; 16]);
        }
        assert!(ChatMessage::decode(&forged.into_bytes()).is_err());
    }

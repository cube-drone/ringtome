...
`sync::local_frontiers` and `documents::materialize`-on-the-save-path are done (see HISTORY).
The rest of the audit, in the order I would take them:

1. **`imaol::rebuild_views` is reachable from ingest** (`sync::ingest_batch`, when eviction
   happened) — a peer can trigger a whole-log replay, decoded and re-validated, into memory.
2. **`imaol::list_entries` is an unbounded HTTP response** (`/api/identity/{root}/entries`) —
   wants the keyset paging `entries_page` already has.
3. **`sync::missing_plan`'s `SELECT DISTINCT author_pubkey, service FROM entries`** — full
   scan per sync exchange, answering a question `chain_heads` also holds.
4. **`imaol::all_entry_bytes`** — whole log into memory for journal backfill at open; fine as
   a one-time recovery path, worth streaming if journals get large.
5. **`imaol::entries_of_type` is a footgun**: no watermark, no bound. Every caller today is an
